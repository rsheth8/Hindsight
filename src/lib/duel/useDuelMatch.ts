...
import { commitDuel, forfeitDuel, getDuel, getDuelRealtime, submitDuelRebuttal } from "./client";
...
export function useDuelMatch(deviceId: string) {
  ...
  const poll = useCallback(async () => {
    const current = matchRef.current;
    if (!current || !needsLiveSync(current.state) || !deviceId) return;
    await refresh();
  ...
  useEffect(() => {
    ...
    async function connectAbly() {
      ...
      realtime.connection.on("disconnected", () => {
        if (cancelled) return;
        // brief disconnect — Ably reconnects; poll only if it stays down
        setTimeout(() => {
  ...
  return { match, setMatch, error, setError, busy, opponentPresence, commit, submitRebuttal, forfeit, clear };
}
