"use client";
/**
 * Live-ish duel state via REST polling. While a match waits for an opponent or a
 * round is active, we poll every ~1.5s so both players see each other lock in
 * near-instantly. Swap for an Ably subscription once the server mints tokens.
 */
import { useCallback, useEffect, useRef, useState } from "react";
import { commitDuel, forfeitDuel, getDuel } from "./client";
...
export function useDuelMatch(deviceId: string) {
  ...
  const poll = useCallback(async () => {
    ...
    if (!current || !isLive(current.state) || !deviceId) return;
    try {
  ...
  useEffect(() => {
    stopPolling();
    if (match && isLive(match.state)) timer.current = setTimeout(poll, POLL_MS);
    return stopPolling;
  ...
  return { match, setMatch, error, setError, busy, commit, forfeit, clear };
}
