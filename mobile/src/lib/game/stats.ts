/**
 * Personal bests & weekly stats — local Rank tab content.
 */
...
function weekCalibration(entries: JournalEntry[]): number | null {
  ...
  return Math.round(entries.reduce((a, h) => a + calibScore(h.brier), 0) / entries.length);
}
...
export function personalBests(history: JournalEntry[], longestStreak: number): PersonalBests {
  ...
  for (const entries of byWeek.values()) {
    ...
    if (s !== null && (bestWeekCalibration === null || s > bestWeekCalibration)) bestWeekCalibration = s;
  }
